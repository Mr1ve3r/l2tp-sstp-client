...
import io.github.mr1ve3r.combined.engine.sstp.cipher.hashMd4
import io.github.mr1ve3r.combined.engine.sstp.extension.sum
...
internal class MsChapV2Client(private val bridge: SstpBridge) {
    ...
    internal fun processChallenge(challenge: ChapValueNameField): ChapValueNameField {
        challenge.value.copyInto(serverChallenge)

        prepareClientResponse()

        return ChapValueNameField().also {
            it.name = bridge.config.username.toByteArray(Charsets.US_ASCII)
    ...
    internal fun prepareHlak() {
        ...
        bridge.state.hlak = MessageDigest.getInstance("SHA-1").let {
            it.update(hashMd4(hashMd4(passArray)))
            it.update(clientResponse)
            it.update(magic1)
